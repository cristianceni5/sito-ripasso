...
void renderHUD(GameCharacter &hero) {
    ...
    std::cout << "Position of character: " << hero.getPosX() << ", " << hero.getPosY();


    // If the hero has a weapon, and i use the getWeapon not null method for that, print the weapon's strength
    if (hero.getWeapon() != nullptr) {
        std::cout <<" aaddaassWeapon's strength:" << hero.getWeapon()->getStrength() << std::endl;
    }
